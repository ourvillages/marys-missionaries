<template>
  <div class="flex items-center space-x-4">
    <div class="lg:max-w-[336px] w-full flex items-center relative px-5 py-3 border border-[#0c66ee] rounded-xl">
      ...
      <input type="text"
        class="w-full text-lg font-medium text-right border-none ring-0 focus:outline-none focus:ring-0" name="url"
        :value="value" />
    </div>
    <div class="relative w-full max-w-[106px] sm:max-w-[159px]">
      ...
      <transition name="transform-fade-down">
        <ul v-if="openDropdown"
          ...
          tabindex="-1">
          <li v-for="exchange in exchanges" id="listbox-option-0" :key="exchange.name"
            class="text-gray-900 cursor-default select-none relative px-3 sm:px-5 py-2" role="option">
