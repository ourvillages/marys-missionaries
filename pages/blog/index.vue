...
const posts = await getItems<BlogItem[]>({ collection: 'blog' })
</script>
  ...
  <div class="h-screen">
    <div class="px-4 py-16 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8 lg:py-20">
      ...
      <div class="grid gap-5 lg:grid-cols-3 sm:max-w-sm sm:mx-auto lg:max-w-full" v-for="post in posts">
        <div class="overflow-hidden transition-shadow duration-300 bg-blue-100 px-4 rounded-lg">
          <div class="py-5">
            <p class="mb-2 text-xs font-semibold text-gray-600">{{ format(new Date(post.date_created), 'P') }}</p>
            <NuxtLink :to="`/blog/${post.id}`" class="inline-block mb-3 text-black transition-colors duration-200 hover:text-deep-purple-accent-700">
              <p class="text-2xl font-bold leading-5 text-blue-800">{{ post.title }}</p>
            </NuxtLink>
